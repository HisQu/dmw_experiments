...
- Check these modules first:
  - `dmw_experiments.shared.config`
  - `dmw_experiments.shared.utils`
  - `apprc`
  - `apprc.cli`
- If a helper is broadly reusable, place it in the appropriate shared module.
- Put reusable runtime, analysis, artifact, configuration, and supervision code
  below `dmw_experiments.shared`.
- Put scientific conditions, study runners, and study-specific analysis below
  `dmw_experiments.studies.<study_name>`.
- Put long-form docs in `docs/`:
  ...
  - one synchronized scientific overview per study in `docs/studies/`
...
- App configuration belongs in `dmw_experiments.shared.config.owners` and should use AppRC `rc.Config` classes with `rc.field(...)` declarations.
...
- Import `dmw_experiments`-owned utility helpers through the facade: `import dmw_experiments.shared.utils as ut`.
